...
/// → debounced zoom-preserving reload) runs end to end. A `ControlAPITestCase` subclass in its own
/// file, sharing the launch + socket harness with the other `Control*UITests` suites.
@MainActor
final class AppearanceFlipUITests: ControlAPITestCase {
    func testAppearanceFlipPreservesFontZoom() throws {
        ...
        XCTAssertTrue(pollFirstSessionFontSize(until: { $0 != nil }, timeout: 8),
                      "zooming should persist a per-session fontSize override")
        let zoomed = try XCTUnwrap(firstSessionFontSize())
    ...
    func testDebugAppearanceRejectsUnknownSide() throws {
