...
// MARK: - Tree and window projection nodes

/// The read-side node types the `tree` and `window.list` responses are built from. Split out of
/// `ControlProtocol.swift` for the file size limit.
...
public struct ControlSessionNode: Codable, Sendable, Equatable {
    ...
    /// The id of the WINDOW that owns this session; nil/omitted in a host-produced tree that projects no
    /// window. The spawned shell's `AGTERM_WINDOW_ID` is a spawn-time snapshot that goes stale on a
    /// `session.move --to-window`, so this — rebuilt per request — is the source of truth.
    public let windowId: String?
    /// The id of the WORKSPACE that owns this session, the workspace node it is nested under. Stamped on the
    /// node so a caller who found a session by id knows its owner without walking back up the tree;
    /// `AGTERM_WORKSPACE_ID` goes stale on any `session.move` the same way `AGTERM_WINDOW_ID` does. Present
    /// exactly when `windowId` is — one ownership stamp, not two independent ones.
    ...
    public init(id: String, name: String, cwd: String, title: String? = nil, active: Bool, split: Bool,
                ...
                windowId: String? = nil, workspaceId: String? = nil) {
        self.id = id
        self.name = name
        self.cwd = cwd
        self.title = title
...
public struct ControlTree: Codable, Sendable, Equatable {
    ...
    /// The id of the window this tree projects. The tree has always been a SINGLE-window projection without
