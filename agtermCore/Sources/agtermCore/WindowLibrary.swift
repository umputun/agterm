...
public final class WindowLibrary {
    ...
    public func controlWindowNodes(geometry: (WindowInfo.ID) -> ControlWindowFrame? = { _ in nil }) -> [ControlWindowNode] {
        let active = activeWindowID
        return windows.map {
            // reach each open store for its auto-follow timeout + sidebar visibility (per-window state); a
            // closed window has no store and reports nil for both. The frame comes from the app-side closure.
