...
- `jq`
- one window per project, each with a distinct name. Create them from the Window menu (New Window, then Rename Window…) or with `agtermctl window new "Project A"`.
...
The script is optional convenience. Window ▸ Open Window already lists every window by name with a checkmark on the open ones, and ⌘` cycles them. What the script adds is the parking: the others go to the Dock instead of staying stacked behind the one you raised.
...
Closed windows are skipped when parking, because `window minimize` requires an open window and answers `window not open` otherwise. A window that cannot be parked is reported on stderr and the script carries on, so one awkward window does not stop the switch.
...
Minimize is rejected on a window in native full screen. AppKit does nothing on `miniaturize` there, so agterm answers `cannot minimize a full-screen window` rather than claiming success. Exit full screen, or use `agtermctl window fullscreen` first.
