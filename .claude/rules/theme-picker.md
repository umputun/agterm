...
- **Appearance sync = two theme slots + an explicit toggle; emission is the RAW dual value.**
  ...
  libghostty resolves the active side itself at runtime: `viewDidChangeEffectiveAppearance` calls `ghostty_surface_set_color_scheme` + `ghostty_app_set_color_scheme` (which record the new conditional state and emit a soft `reload_config`) and posts `.agtermSystemAppearanceChanged`; `SettingsModel.appearanceChanged` (guarded on following + both slots set) triggers `reloadConfigClearingSessionZoom` DIRECTLY — the raw dual file text is stable across flips, so `writeGhosttyConfig`'s text-diff would otherwise skip the reload (see the libghostty rule for the mechanism).
  ...
  The one surviving dual PARSE is `AppSettings.dualThemeSides` — used to migrate a legacy stored dual `theme` (`SettingsStore.load` → `migratingLegacyDualTheme`) and to pick the active side for the sidebar selection colors (`GhosttyApp.resolveSelectionColors`, which reads the raw `theme` line back out of the config); `ThemeResolution` is gone.
  ...
  A palette PREVIEW writes the CURRENT-appearance slot (`previewTheme`/`commitTheme` pick the dark slot while following in dark mode, else `theme`), so the on-screen render reflects it and Esc restores the captured slot value.
  COMMIT (`AppActions.commitThemePreview` → `SettingsModel.commitTheme(_:)`, save-only) just persists that slot — no dual recompose, since the two slots are separate fields.
