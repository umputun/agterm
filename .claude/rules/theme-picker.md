...
- **Appearance sync = two theme slots + an explicit toggle; emission is the RAW dual value.**
  ...
  libghostty resolves the active side itself at runtime: `viewDidChangeEffectiveAppearance` calls `ghostty_surface_set_color_scheme` + `ghostty_app_set_color_scheme` (which record the new conditional state and emit a soft `reload_config`) and posts `.agtermSystemAppearanceChanged` with the POSTING view's `isDark` in userInfo; `SettingsModel.appearanceChanged` (guarded on following + both slots set, AND on the posted side actually differing from the last config feed — the notification also fires per surface at ATTACH, so a new session must not trigger a spurious reload) triggers `reloadConfigPreservingSessionZoom` DIRECTLY — the raw dual file text is stable across flips, so `writeGhosttyConfig`'s text-diff would otherwise skip the reload.
  ...
  The side is never re-read from `NSApp.effectiveAppearance` at receive time — it can lag the views around sleep/wake, which stuck the terminal on the old theme after wake (see the libghostty rule for the mechanism; `AppearanceFlipUITests` pins the zoom-preserving flip via the UI-test-only `debug.appearance` seam).
  ...
  The one surviving dual PARSE is `AppSettings.dualThemeSides` — used only to pick the active side for the sidebar selection colors (`GhosttyApp.resolveSelectionColors`, which reads the raw `theme` line back out of the config); `ThemeResolution` and the legacy dual-string migration are gone.
