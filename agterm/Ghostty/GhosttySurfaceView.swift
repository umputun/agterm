...
final class GhosttySurfaceView: NSView, TerminalSurface {
    ...
    var dashboardFontOverride: Double? {
        didSet {
            ...
            // CLEARING the override (non-nil -> nil) restores the surface to the session's own font. When
            // session.fontSize is nil (the session follows the Settings default), that restore returns the
            // cell to the default and fires a CELL_SIZE report; persisting it would PIN the session to the
            // default and stop it following a later change. Suppress the restore's report — the flag clears
            // one runloop turn later, after the (synchronous) CELL_SIZE report the update_config enqueued.
            guard dashboardFontOverride == nil, oldValue != nil else { return }
            suppressFontReport = true
            DispatchQueue.main.async { self.suppressFontReport = false }
        }
    }

    /// One-shot latch that suppresses the CELL_SIZE font-report generated by CLEARING `dashboardFontOverride`
    /// (see its `didSet`), so restoring the dashboard grid font never persists into `session.fontSize`.
    private var suppressFontReport = false
    ...
    func reportFontSize() {
        ...
        // while a dashboard font override is active — OR just after it was cleared (the restore's own
        // CELL_SIZE round-trip, see suppressFontReport) — don't persist: it would otherwise write the
        // transient dashboard size into session.fontSize, or pin a default-following session on close.
        guard dashboardFontOverride == nil, !suppressFontReport else { return }
