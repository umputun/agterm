...
- A live-preview theme picker (View ▸ Select Theme…, the action palette's "Select Theme…", or the `select_theme` keymap action): a fuzzy-search palette of the bundled themes that applies each one to the open terminals **as you navigate** — Enter commits it (and syncs Settings), Esc reverts to the theme you started on. The full theme catalog stays out of the action palette; only the single "Select Theme…" launcher appears there.
