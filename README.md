...
Sidebar session rows support Shift-click range selection and Cmd-click toggling for batch work. Right-clicking inside a multi-selection keeps the batch for Flag/Unflag, Close, and Move to; right-clicking outside narrows to the clicked row. Dragging from a selected row moves the selected sessions as one ordered block. **Copy Name** — under Rename on session and workspace rows — puts the row's name on the clipboard. It copies the name the row derives, so a session that was never renamed yields its terminal title or directory basename rather than a blank. Single-target, like Rename itself. **Duplicate Session** — in a single session's context menu, under Copy Name — opens a fresh session in the same workspace, right after that one, in its current directory (a plain new shell: only the directory carries over, nothing else about the session does).
