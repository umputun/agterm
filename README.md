...
Sidebar session rows support Shift-click range selection and Cmd-click toggling for batch work. Right-clicking inside a multi-selection keeps the batch for Flag/Unflag, Close, and Move to; right-clicking outside narrows to the clicked row. Dragging from a selected row moves the selected sessions as one ordered block. **Copy Name** — directly under Rename, on both session and workspace rows — puts the row's sidebar label on the clipboard; with several sessions selected it copies **Copy Names**, one per line. It copies what the row shows, so a session that was never renamed yields its terminal title or directory basename rather than a blank. **Duplicate Session** — in a single session's context menu, under Copy Name — opens a fresh session in the same workspace, right after that one, in its current directory (a plain new shell: only the directory carries over, nothing else about the session does).
