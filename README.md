...
The same recently-used history decides where you land when you close the session you are in: agterm returns you to the session you were most recently working in, not to whichever row happens to sit next to the one that closed. The pick stays inside the closing session's workspace and respects an active focus or flagged filter, widening beyond the workspace only when the close leaves nothing there to return to; if no recent session qualifies, it falls back to the adjacent one.
