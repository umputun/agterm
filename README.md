...
A custom command also resolves its binaries against the app's GUI `PATH` — the launchd default, not your shell's: nothing `path_helper` or your shell profile adds is there, `/opt/homebrew/bin` included — so a bare Homebrew or other non-default binary fails with exit 127. Give an absolute path, or wrap the line in `zsh -lc '…'` to pick up your login `PATH`. The same applies to the program an overlay or scratch terminal runs.
