...
- **Agent-status glyph does not update.** Install the hooks from Help ▸ Install Agent Status Hooks…. For shell-integrated agents, start a fresh shell so the `source` line added to your shell rc takes effect. For Pi, restart it or run `/reload` so it loads `~/.pi/agent/extensions/agterm-status.ts`; Pi status is only installed when `~/.pi/agent` already exists. For OpenCode, the installer requires `~/.config/opencode`, detects the major through `opencode --version`, and installs `~/.config/opencode/plugins/agterm-status.js` (v1) or `~/.config/opencode/plugins/agterm-v2/tui.js` (v2). If detection fails, choose the version in the dialog. Re-run the installer after switching majors, then restart OpenCode. The v2 plugin reports the CLI's selected session and its descendants, not background tabs or headless runs. The hooks call `agtermctl session status`, so `agtermctl` must resolve first (see above).
