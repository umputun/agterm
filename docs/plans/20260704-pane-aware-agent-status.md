...
- [ ] add `let statusPane: String?` to `ControlSessionNode` (Codable optional, omitted when nil) + its init param, defaulting nil so existing call sites compile
- [ ] add `pane: StatusPane?` to `ControlSessionStatusUpdate` (`ControlModes.swift`) + init
- [ ] in `ControlDispatcher` `.sessionStatus`: parse `request.args?.pane` into `StatusPane` (nil→nil); a non-nil unknown value returns the same error text as `--pane` validation (`--pane must be left, right, or scratch`); pass it into the update
- [ ] write tests: `ControlSessionNode` round-trips with `statusPane` and omits it from JSON when nil; `ControlSessionStatusUpdate` carries pane
- [ ] write tests: dispatcher `.sessionStatus` — valid pane populates the update, invalid pane returns the validation error (status unchanged), nil pane leaves it nil
...
- [ ] add `@Option --pane` to the `Status` subcommand; `validate()` via the shared `validatePaneArgument` (`left|right|scratch`); thread into `ControlArgs.pane` in `makeRequest()` (mirror `session type --pane`)
- [ ] update the `validatePaneArgument` godoc (`SessionCommands.swift:5`, currently "type, text") to add `status` as a caller
- [ ] write tests: `session status blocked --pane right` maps to `ControlArgs.pane == "right"`; omitted → nil; invalid pane rejected at parse
- [ ] run `cd agtermCore && swift test`, `make lint` — must pass before next task
...
- [ ] make `surfaceEnv(for:)` inject the matching `AGTERM_PANE` per factory via `SurfaceEnvironment.session(pane:)`: `makeSurface` `.left`, `makeSplitSurface` `.right`, `makeScratchSurface` `.scratch`; `makeOverlaySurface` injects none
- [ ] change `GhosttySurfaceView.onUserInputClearsStatus` to take `isEscape: Bool`; `keyDown` (`+Input.swift:51-54`) fires it UNCONDITIONALLY and no longer reads `session?.agentIndicator` (the closure now owns the decision, so the scratch — which has no `view.session` — is covered)
- [ ] wire the closure in ALL THREE factories (`makeSurface`→`.left` at :216, `makeSplitSurface`→`.right` at :318, `makeScratchSurface`→`.scratch`, currently none), each capturing `store`+`sessionID`+its pane: `if store.session(withID: sessionID)?.agentIndicator.clearedBy(pane: <thisPane>, isEscape:) == true { store.setAgentIndicator(AgentIndicator(), forSession: sessionID) }`
- [ ] verify `destroySurface()` still nils `onUserInputClearsStatus` (the retain-cycle break); do NOT add a `statusPane` property to `GhosttySurfaceView`
- [ ] `make build` succeeds; `make lint` clean — must pass before next task *(the clear DECISION is unit-tested in Task 1; the wiring + survival/self-clear are covered by the e2e in Task 10)*
...
- [ ] add a shared "reveal the active session's blocked pane" step reading `session.agentIndicator.statusPane`: `right` → set `splitFocused = true` then focus `session.activeSurface` (do NOT gate the focus target on `hasSplit` — that breaks the promoted survivor); `scratch` → `if !session.scratchActive { store.toggleScratch(id) }` then focus the topmost surface; `left`/nil → the main pane
- [ ] wire it into `autoFollowed(_:)` (replace the plain `focusActiveSession` for the followed session)
- [ ] wire it into attention-nav (`selectNextAttentionSession`, shared by `session.go next-attention`) so the post-select focus targets the blocked pane
- [ ] verify against `AppStore.closePrimaryPane`: a `right`-tagged block on a promoted survivor (`surface == nil`, survivor in `splitSurface`, `hasSplit == false`) still focuses the survivor; confirm the parked-on-session auto-follow suppression is unchanged
- [ ] `make build` succeeds; `make lint` clean — must pass before next task *(behavior covered by the e2e in Task 10)*

### Task 10: XCUITest e2e — reveal the blocked pane

**Files:**
- Modify: `agtermUITests/AutoFollowUITests.swift` *(or a new `PaneAwareStatusUITests.swift`, a `ControlAPITestCase` subclass, if `AutoFollowUITests` nears the 2000-line budget)*

- [ ] e2e: set `session.status blocked --pane right` on a split session, trigger nav, assert the split (right) pane is focused/revealed
- [ ] e2e: with the split hidden, nav reveals it (the right pane shows maximized)
- [ ] e2e: `session.status blocked --pane scratch` with the scratch hidden, nav shows + focuses the scratch (the `toggleScratch` path)
- [ ] e2e: typing in the main pane does NOT clear a `right`- or `scratch`-tagged block (survival — the core fix)
- [ ] e2e: typing in the scratch DOES clear a `scratch`-tagged block (self-clear parity — the Task 8 closure)
- [ ] run the e2e; if any is genuinely flaky, add a `⚠️` note here and keep the assertion — do NOT weaken it
- [ ] `make build`, full `swift test`, `make lint` — must pass before next task
...
- [ ] verify `tree` reports `statusPane`; `agtermctl session status blocked --pane right` round-trips
- [ ] verify the scope fence held (sidebar glyph one-per-session; attention/auto-follow session-level; `SessionSnapshot` untouched)
- [ ] run full `cd agtermCore && swift test`, `make build`, `make lint --strict`, and the XCUITest e2e
- [ ] manual dev-instance check of the three reveal cases (see Post-Completion)
